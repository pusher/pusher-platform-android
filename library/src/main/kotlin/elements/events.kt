...
sealed class SubscriptionMessage {
    companion object Factory {
        fun fromRaw(messageString: String): SubscriptionMessage {
            val message = BaseSubscription.GSON.fromJson(messageString, Array<JsonElement>::class.java)
            return when (message[0].asInt) {
                0 -> ControlEvent(body = message[1].asString)
                1 -> SubscriptionEvent(eventId = message[1].asString, headers = BaseSubscription.GSON.fromJson(message[2], Map::class.java) as Headers, body = message[3])
